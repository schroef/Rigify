...
class RigLayers(bpy.types.Panel):
    ...
    #Add toggleFunction
    def toggleIkFk(self,layer, toggle):
        actObj = bpy.context.active_object.data
        actObj.layers[layer]=True if toggle else False
...
'''
    ...
    limbs = ["Arm.L (IK)","Arm.R (IK)", "Arm.L (FK)","Arm.R (FK)","Leg.L (IK)","Leg.R (IK)","Leg.L (FK)","Leg.R (FK)"]
    code += "        pbone = context.active_object.pose.bones\n"
    upArmL = "['upper_arm_parent.L']['IK_FK']"
    upArmR = "['upper_arm_parent.R']['IK_FK']"
    thiLegL = "['thigh_parent.L']['IK_FK']"
    thiLegR = "['thigh_parent.R']['IK_FK']"
    for key in keys:
        code += "\n        row = col.row()\n"
        i = 0
        for l in rows[key]:
            if i > 3:
                code += "\n        row = col.row()\n"
                i = 0
            # Checks slider value > hide button with 0.0 or 1.0    
            if (l[0] == limbs[0]):
                code += "        if (pbone%s != 1.0):\n    " % upArmL
            if (l[0] == limbs[1]):
                code += "        if (pbone%s != 1.0):\n    " % upArmR
...
class ScriptGenerator(base_generate.GeneratorPlugin):
    ...
    def finalize(self):
        ...
        #Below layers so panels doesnt jump around when items are hidden
